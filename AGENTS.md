...
Target: a full-featured **single-band** compressor, with DSP held in a fixed-capacity band
array whose active count can grow for a future multiband version (hence the project name)
without rewriting the per-band processor.

## Current state and roadmap

Phases 0–7 are complete: the plugin has a validated CLAP lifecycle, all 20 parameters,
the complete single-band DSP and routing path, stereo/mono port configurations, state,
latency/tail reporting, and the fixed-size Cocoa/OpenGL/nanovg GUI. The correctness,
audio-polish, GUI/hygiene, review, and final-cleanup batches recorded in `PLAN.md` are also
complete.

The only remaining implementation phase in the current plan is **Phase 8 — Polish**:
...
3. Per-event parameter modulation, followed by restoring `MODULATABLE` only on parameters
   whose modulation is actually handled.
4. GUI resizing, including real `set_scale`/size handling and scalable layout/rendering.

Code signing and distribution packaging are deliberately deferred until the plugin has
spent time in real sessions. Windows/Linux GUI support, VST3/AU wrappers, and the eventual
multiband expansion are longer-term work, not part of Phase 8. `PLAN.md` contains the
completed phase history and is the roadmap authority; keep its next-work section aligned
with this summary.

Implemented standard CLAP extensions today: `audio-ports`, `audio-ports-config`, `params`,
`state`, `latency`, `tail`, `gui`, and `timer-support`. The meter and parameter-mirror
interfaces in `extensions.odin` are private test/readback interfaces, not CLAP extensions.
...
| `clap-validator` | `/Users/gmb/code/bin/clap-validator` | 0.3.2, official test suite |
...
multicomp/
  ...
  CLAUDE.md          a symlink to AGENTS.md, so both conventions find the same doc
...
```sh
odin build src/plugin -collection:proj=/Users/gmb/code/foesoft/multicomp
```
...
that meters start at silence, that event-list edge cases are retained, and that a state
round trip reaches both the GUI mirror and DSP while announcing a latency change when a
loaded lookahead disagrees with the latched one. Use it whenever DSP, parameter, state,
routing, port, latency, or meter behaviour changes. It resolves parameters **by name**, so
reordering `Param_Id` cannot silently invalidate the checks.
...
plugin tests, **58** offline audio checks and **38** GUI checks. The 5 skips are note-port
and preset-discovery tests, correctly skipped for an audio effect that has neither yet.
...
- `clap-validator validate` must pass with **0 failures and 0 warnings**, `odin test src/dsp`
  and `odin test src/plugin -collection:proj=<repo root>` must be fully green, and
  `./build.sh --offline --gui` must report all checks passed, before any change is called
  done. Report the actual counts.
